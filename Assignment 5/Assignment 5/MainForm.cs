...
public partial class MainForm : Form
{
    ...
    private void buttonEdit_Click(object sender, EventArgs e)
    {
        if (listBoxIdName.SelectedIndex != -1)
        {
            ...
            using (ContactForm selected = new ContactForm(selectedCustomer.GetContact(), "Edit customer " + selectedCustomer.GetName()))
            {
                ...
                {
                    manager.ReplaceCustomer(selectedIndex, selected.GetCustomer());
    ...
    {
        listBoxIdName.Items[i] = String.Format(stdDetails, currentCustomer.idCustomer, currentCustomer.GetName(),
                 currentCustomer.mobile, currentCustomer.emailBuissness);
    }
    // Display customer contact information after adding a 
    // new customer 
    private void DisplayCustomerInformation()
    {
        listBoxIdName.Items.Add(String.Format(stdDetails, currentCustomer.idCustomer, currentCustomer.GetName(),
                 currentCustomer.mobile, currentCustomer.emailBuissness));
    }
    #endregion
    #region UNUSED 
